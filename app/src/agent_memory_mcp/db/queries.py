...
async def get_recent_messages_for_domains(
    engine: AsyncEngine,
    domain_ids: list[UUID],
    days: int = 30,
    per_domain_limit: int = 6,
) -> dict[UUID, list[dict]]:
    """Recent non-noise messages for many domains in ONE windowed query, so
    callers (e.g. the label classifier) avoid a get_recent_messages-per-domain
    round-trip storm. Returns domain_id → newest-first dicts, capped per domain."""
    if not domain_ids:
        return {}
    rn = (
        func.row_number()
        .over(partition_by=messages.c.domain_id, order_by=messages.c.msg_date.desc())
        .label("rn")
    )
    sub = (
        select(messages, rn)
    ...
    stmt = (
        select(sub)
        .where(sub.c.rn <= per_domain_limit)
        .order_by(sub.c.domain_id, sub.c.msg_date.desc())
    )
    out: dict[UUID, list[dict]] = {}
    async with engine.begin() as conn:
        for r in (await conn.execute(stmt)).mappings().all():
